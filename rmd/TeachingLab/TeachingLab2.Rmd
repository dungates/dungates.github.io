...
test <- ed_data_join %>% left_join(race_data_join, by = c("GEOID", "NAME")) #%>%
  # mutate(race_percent = 100*estimate_percent) %>%
...
race_data_join %>%
  ...
  scale_fill_viridis_c(direction = -1) +
  scale_color_viridis_c(direction = -1) +
  labs(title = "Racial geography of the US in 2018",
       caption = "Source: American Community Survey",
       fill = "Percent\nby County") +
  theme_void() +
...
states_df <- states_sf %>%
  left_join(race_education_state, by = c("state_name" = "state"))
...
ggplot(states_df, aes(fill = education_percent)) +
  ...
  scale_color_manual(values = "black") +
  scale_alpha_manual(values = 0.2) +
  labs(title = "Educational Geography of the US in 2018",
       caption = "Source: American Community Survey",
       fill = "Percent\nby State") +
  theme_void() +
  theme(plot.title = element_text(hjust = 0.5),
...
# 
# pivot <- ed_data_state %>%
#   pivot_wider(names_from = "education_level", values_from = "estimate")
#   
# pivot2 <- race_data_state %>%
#   pivot_wider(names_from = "race", values_from = "race_percent")
# 
# state_matrix <- left_join(pivot, pivot2, by = c("NAME")) %>% rename(State = NAME)
# 
# state_matrix_final <- as.matrix(state_matrix)
...
# races_education_vars <- c("White", "Black", "Asian", "Hispanic", "Less than 9th Grade", "9th to 12th grade, no diploma",
#            "High school graduate", "Some college, no degree", "Associate's degree", "Bachelor's degree", 
#            "Grad/pro degree")
# dimnames(state_matrix_final) <- list(Race = levels(race_data_state$race),
#                                      Education = levels(ed_data_state$education_level))
# rownames(state_matrix_final) = paste0("State", 1:51)
# colnames(state_matrix_final) = paste0("Education", 1:12)
# colnames(state_matrix_final) = paste0("Race", 9:12)
# groupColors <- c("#000000", "#FFDD89", "#957244", "#F26223")


# Build the chord diagram:
# p <- chorddiag(state_matrix_final, groupColors = groupColors, groupnamePadding = 20)
# p
