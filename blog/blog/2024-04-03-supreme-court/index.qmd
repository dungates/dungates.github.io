...
every_4_years_pres_sup <- modern_supreme_presidents |>
  ...
  dplyr::ungroup() |>
  dplyr::mutate(president = ifelse(n < 2, 
                                   gsub("(?<=[A-Z])[^A-Z]+", "\\.", president, perl = TRUE),
                                   president))

(as.numeric(lubridate::today() - as.Date("1991-10-15")) / 365.25) |> 
  round(digits = 0) |>
  paste(... = _, "years") -> days_clarence_thomas

every_4_years_pres_sup |>
  ggplot2::ggplot(aes(x = every_4, y = n, fill = party, group = party)) +
  ggplot2::geom_col(position = ggplot2::position_dodge(), alpha = 0.8) +
  ggplot2::geom_text(aes(x = every_4,
                         y = n,
                         label = president, 
                         group = party),
                     hjust = 1.1,
  ...
  ggplot2::coord_radial(rotate_angle = TRUE, expand = FALSE) +
  DGThemes::theme_duncan(
    grid_col = "gray92"
  ) +
  ggplot2::theme(legend.title = ggplot2::element_blank(),
                 axis.line.theta = ggplot2::element_line(colour = "black", linewidth = 0.1),
                 axis.text.x = ggplot2::element_text(margin = margin(0,0,0,10)))
```

The longest sitting Supreme Court justice is currently Clarence Thomas, bucking the trend, sitting for an impressive `r days_clarence_thomas`. Let's zoom in on just the period since his nomination to get one last look at what the nominations of the court have looked like by party in recent history.

```{r}
#| fig.width: 10
#| fig.height: 10
every_4_years_pres_sup |>
  dplyr::filter(every_4 > as.Date("1985-01-01")) |>
  ggplot2::ggplot(aes(x = every_4, y = n, fill = party)) +
  ggplot2::geom_col(aes(fill = party), position = ggplot2::position_dodge2(), alpha = 0.8) +
  ggplot2::geom_text(aes(x = every_4,
                         y = n,
                         label = president, 
                         group = party),
...
Its clear in recent years that the court has had more republican nominations in recent years, and those republican nominations are replacing nominees from the Democratic party from years prior! 
